...
class FTF(Scheduler):
    ...
    def schedule(self, jobs, gpus, cluster_copy=None):
        ...
        jobs.sort(
            key=lambda job: 
            (-job.job_priority, job.finish_time_fair_metric(runner_time, fair=self.fair), job.job_arrival_time))
            #(-job.job_priority, job.attained_service(), job.job_arrival_time))
...
#        # If real cluster deployment, launch each job
#        if not self.simulate:
#           self.deploy_jobs_round()
