...
class SRTF(Scheduler):
    ...
    def schedule(self, jobs, gpus, cluster_copy=None):
        ...
        jobs.sort(
            key=lambda job: 
            (-job.job_priority, job.remaining_duration(fair=self.fair), job.job_arrival_time))
        ...
        #    self.logger.info(skipped_jobs)
        #    sys.exit(1)
        #self.logger.info("[SCHED now] jobs : {}, running ={}".format(len(jobs), len(self.running_jobs)))
        #for j in self.running_jobs: 
        #   self.logger.info("{}:{}cpu:{}GB".format(str(j), j.cpus, j.mem))  
