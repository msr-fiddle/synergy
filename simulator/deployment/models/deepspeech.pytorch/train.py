...
parser.add_argument('-j', '--num-workers', default=4, type=int, help='Number of workers used in data-loading')
...
parser.add_argument('--chk_dir', default='./chk/', type=str)
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        args.gpu_rank = args.local_rank
        args.world_size = int(os.environ['WORLD_SIZE'])

    if args.distributed and args.gpu_rank is None:
        args.gpu_rank = args.local_rank
    ...
    if False and os.path.exists(chk_path):  # Starting from previous model
        print("Loading checkpoint model %s" % chk_path)
        package = torch.load(chk_path, map_location=lambda storage, loc: storage)
        ...
        if not args.finetune:  # Don't want to restart training
            ...
            args.steps_so_far = package.get('steps_so_far')
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if not args.noeval and main_proc and args.checkpoint:
                file_path = '%s/deepspeech_%d.pth.tar' % (save_folder, epoch + 1)
                torch.save(DeepSpeech.serialize(model, optimizer=optimizer, epoch=epoch, loss_results=loss_results,
                                            wer_results=wer_results, cer_results=cer_results),
                       file_path)
        if not args.noeval and main_proc and (best_wer is None or best_wer > wer):
    ...
    if args.synergy:
        print("Saving checkpoint model %s" % chk_path)
        torch.save(DeepSpeech.serialize(model, optimizer=optimizer, epoch=epoch, iteration=i,
                                                steps_so_far = args.steps_so_far,
                                                loss_results=loss_results,
                                                wer_results=wer_results, cer_results=cer_results), chk_path)
