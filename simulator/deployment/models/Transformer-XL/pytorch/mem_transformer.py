...
from utils_tx.log_uniform_sampler import LogUniformSampler
from utils_tx.log_uniform_sampler import sample_logits
from utils_tx.proj_adaptive_softmax import ProjectedAdaptiveLogSoftmax
...
class RelMultiHeadAttn(nn.Module):
    ...
    def _parallelogram_mask(self, h, w, left=False):
        ...
        if left:
            ...
            return mask.flip(0).type(torch.uint8) 
    ...
    def _forward(self, dec_inp, mems=None):
        ...
        if self.same_length:
            ...
            dec_attn_mask = (torch.triu(all_ones, 1+mlen)
                             + torch.tril(all_ones, -mask_shift_len)).type(torch.uint8) 
        else:
            dec_attn_mask = torch.triu(
                word_emb.new_ones(qlen, klen), diagonal=1+mlen).type(torch.uint8)  
