...
def parse_args():
    ...
    print("Config : {}, {}".format(config_args.config, config_args.config_file))
    ...
    training.add_argument('--max_step', type=int, default=400000000,
    ...
    training.add_argument("--elapsed_iters", default=0, type=int)
    ...
    training.add_argument("--sched-addr", default='127.0.0.1', type=str) 
    training.add_argument("--sched-port", default='14000', type=str) 
    training.add_argument("--log-dir", default='./', type=str) 
    training.add_argument('--job-id',  default=0, type=int)
    training.add_argument('--chk_dir', default='./chk/', type=str, metavar='PATH', help='path to latest checkpoint (default: ./chk)') 
...
def save_checkpoint(args, model, model_config, optimizer, scheduler, vocab,
                    train_step, best_val_loss, work_dir, name='model.chk'):
    if args.fp16:
...
def train(tr_iter, va_iter, model, para_model, model_config, optimizer,
    ...
    if not args.synergy:
        ...
        train_iter = tr_iter.varlen() if args.varlen else tr_iter
    ...
    save_checkpoint(args, model, model_config, optimizer,
                    scheduler, vocab, train_step,
                    best_val_loss, args.chk_dir, name='model.chk')
...
def main():
    ...
    if not os.path.exists(args.chk_dir):
    ...
    print("CHK Path = {}, epochs = {}".format(chk_path, args.epochs))
    ...
    if args.restart:
        ...
        print("Steps so far = {}, max_iters={}".format(args.steps_so_far, args.max_iterations))
    ...
    if args.synergy:
        tr_iter = SynergyIterator(tr_iter, max_iters=args.max_iterations, iters_elapsed=args.steps_so_far)
    ...
    try:
        for epoch in itertools.count(start=1):
            ...
            if args.synergy and tr_iter.exit:
                logging.info('-' * 100)
                logging.info('End of training : steps {}'.format(train_step))
                break

            if train_step == args.max_step:
                logging.info('-' * 100)
                logging.info('End of training : {}, {}'.format(train_step, args.max_step))
                break
            if epoch == args.epochs:
                logging.info('-' * 100)
                logging.info('End of training : {}'.format(args.epochs))
                break
