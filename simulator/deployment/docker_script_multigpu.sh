...
DISTRIBUTED=${SYNERGY_DISTRIBUTED}
MASTER_IP=${SYNERGY_MASTER_IP}
MASTER_PORT=${SYNERGY_MASTER_PORT}
NNODES=${SYNERGY_NNODES}
RANK=${SYNERGY_RANK}
echo "Distributed job with master ip $MASTER_IP, port $MASTER_PORT, nnodes=$NNODES, rank=$RANK"
...
echo $RANK, $GPUS, $CPUS_PER_GPU, $MEM, $ITERS, $ARCH, $BATCH, $GPU_IDS, $CPU_IDS, $LOG_DIR, $CHK_DIR, elapsed=$ELAPSED_ITERS
...
nvidia-docker run \
  ...
  -e "ELAPSED_ITERS="$ELAPSED_ITERS"" \
    ...
    --master_port=$MASTER_PORT  \
    --master_addr=$MASTER_IP  \
    ./models/Transformer-XL/pytorch/train.py \
    --sched-addr "$SCHED_IP" \
    --sched-port "$SCHED_PORT" \
    --job-id "$JOB_ID" \
    --log-dir "$LOG_DIR" \
...
nvidia-docker run \
  ...
  -e "ELAPSED_ITERS="$ELAPSED_ITERS"" \
    ...
    --master_port=$MASTER_PORT  \
    --master_addr=$MASTER_IP  \
    ./models/audio_classification/audio-classify.py \
    --sched-addr "$SCHED_IP" \
    --sched-port "$SCHED_PORT" \
    --job-id "$JOB_ID" \
    --log-dir "$LOG_DIR" \
...
nvidia-docker run \
  ...
  -e "ELAPSED_ITERS="$ELAPSED_ITERS"" \
    ...
    --master_port=$MASTER_PORT  \
    --master_addr=$MASTER_IP  \
    ./models/deepspeech.pytorch/train.py \
    --sched-addr "$SCHED_IP" \
    --sched-port "$SCHED_PORT" \
    --job-id "$JOB_ID" \
    --log-dir "$LOG_DIR" \
