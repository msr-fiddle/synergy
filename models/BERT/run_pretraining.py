...
def create_pretraining_dataset(input_file, max_pred_length, shared_list, args):
    ...
    train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                  batch_size=args.train_batch_size * args.n_gpu, num_workers=args.workers,
                                  pin_memory=True)
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.fp16:
        ...
        if args.loss_scale == 0:
            model, optimizer = amp.initialize(model, optimizer, opt_level="O2", loss_scale="dynamic", min_loss_scale=1.0)
        else:
            model, optimizer = amp.initialize(model, optimizer, opt_level="O2", loss_scale=args.loss_scale, min_loss_scale=1.0)
    ...
    if args.do_train:
        ...
        while True:
            ...
            train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                          batch_size=args.train_batch_size * args.n_gpu, num_workers=args.workers,
                                          pin_memory=True)
            print("NUM STEPS PER EPOCH:{}, batch size: {}".format(len(train_data), args.train_batch_size * args.n_gpu))
