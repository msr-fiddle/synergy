...
#			result_dir="${OUT_DIR}/${arch}_b${batch}_w${workers}_g${num_gpu}_dali_amp_gpu"
			result_dir="${OUT_DIR}/${arch}_b${batch}_w${workers}_g${num_gpu}_dali_amp_cpu_j2"
			...
			CUDA_VISIBLE_DEVICES=4,5,6,7 python -m torch.distributed.launch --nproc_per_node=$num_gpu --master_port=29900 $SRC/pytorch-imagenet.py -a $arch -b $batch --workers $workers --epochs 3 --amp --dali --dali_cpu  $DATA_DIR > stdout_2.out 2>&1
#380000
