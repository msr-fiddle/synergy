...
parser = argparse.ArgumentParser(description='PyTorch Audio Classification using DALI')
parser.add_argument('data', metavar='DIR', nargs='*',
                    help='path(s) to dataset (if one path is provided, it is assumed\n' + 
                    'to have subdirectories named "train" and "val"; alternatively,\n' + 
                    'train and val paths can be specified directly by providing bothpaths as arguments)')      
...
def train(model, epoch):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % log_interval == 0: #print training stats
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}\t Data_t:{}\t Total:{}'.format(
                epoch, batch_idx * len(data), len(train_loader.dataset),
                100. * batch_idx / len(train_loader), loss, data_time, total_time))
