...
for workers in $worker; do
	...
	#$SCRIPTS/gpulog.sh &
	CUDA_VISIBLE_DEVICES=4,5,6,7 python3 -m torch.distributed.launch --nproc_per_node=$num_gpu $SRC/train.py --seed 2 --train-global-batch-size $batchsize --train-loader-workers $worker --epochs 2  --dataset-dir $DATA_DIR > stdout_3.out 2>&1			
	#pkill -f mpstat
	#pkill -f dstat
	#pkill -f free
	#pkill -f gpulog 
